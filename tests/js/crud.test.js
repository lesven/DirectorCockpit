...
import { save } from '../../public/js/store.js';
...
describe('removeEntity() – Grundverhalten', () => {
  it('entfernt das Element aus dem Array wenn confirm = true', () => {
    resetData({ teams: [{ id: 1, name: 'Alpha' }, { id: 2, name: 'Beta' }] });
    removeEntity('teams', 1);
    expect(mockData.teams).toHaveLength(1);
    expect(mockData.teams[0].id).toBe(2);
  });

  it('lässt das Element im Array wenn confirm = false', () => {
  ...
  it('ruft save() auf nach Löschen', () => {
    resetData({ teams: [{ id: 1, name: 'Alpha' }] });
    removeEntity('teams', 1);
    expect(save).toHaveBeenCalledOnce();
  });

  it('ruft save() NICHT auf wenn Benutzer abbricht', () => {
    confirm.mockReturnValue(false);
    resetData({ teams: [{ id: 1, name: 'Alpha' }] });
    removeEntity('teams', 1);
  ...
  it('ruft renderEntity mit dem richtigen Typ auf', () => {
    resetData({ teams: [{ id: 1, name: 'Alpha' }] });
    removeEntity('teams', 1);
    expect(renderEntity).toHaveBeenCalledWith('teams');
  });

  it('nutzt den Eintragsnamen in der confirm-Meldung', () => {
    resetData({ teams: [{ id: 5, name: 'Mein Team' }] });
    removeEntity('teams', 5);
    expect(confirm).toHaveBeenCalledWith(expect.stringContaining('Mein Team'));
  });

  it('nutzt Fallback-Label wenn Name leer', () => {
...
describe('removeEntity("initiatives") – Kaskaden-Löschung', () => {
  it('löscht alle Risks der Initiative mit', () => {
    resetData({
    ...
    removeEntity('initiatives', 10);
    expect(mockData.risks).toHaveLength(1);
    expect(mockData.risks[0].id).toBe(3);
  });

  it('löscht alle Milestones der Initiative mit', () => {
    resetData({
      initiatives: [{ id: 10, name: 'Projekt X' }],
      risks: [],
      milestones: [
  ...
  it('löscht Risks UND Milestones gemeinsam', () => {
    resetData({
      initiatives: [{ id: 10, name: 'Projekt X' }],
  ...
  it('lässt Risks und Milestones anderer Initiativen unangetastet bei Abbruch', () => {
    confirm.mockReturnValue(false);
    resetData({
      initiatives: [{ id: 10, name: 'Projekt X' }],
  ...
  it('ist korrekt wenn keine Risks/Milestones vorhanden', () => {
    resetData({
      initiatives: [{ id: 10, name: 'Projekt X' }],
      risks: [],
      milestones: [],
    });
    expect(() => removeEntity('initiatives', 10)).not.toThrow();
...
describe('removeEntity("teams") – keine Kaskaden-Löschung', () => {
  it('löscht Risks anderer Entities NICHT', () => {
    resetData({
